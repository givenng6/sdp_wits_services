...
class AuthService {
  ...
  signInWithGoogle(String butn) async {
    btn = butn;
    try {
      ...
      for(int i = 0; i < userEmail.length; i++) {
        if (userEmail[i] == '@') {
          ...
          if(emailExtension == '@students.wits.ac.za' && btn == 'SignInAsStaff'){
            await googleSignIn.disconnect();
            String msg = 'You Should Sign In As A Student';
            Fluttertoast.showToast(msg: msg, fontSize: 18);
            return const App();
          }
          else if(emailExtension == '@wits.ac.za' && btn == 'SignInAsStudent'){
            await googleSignIn.disconnect();
            String msg = 'You Should Sign In As A Staff';
            Fluttertoast.showToast(msg: msg, fontSize: 18);
            return const App();
          }
          else if(emailExtension != '@students.wits.ac.za' && btn == 'SignInAsStudent'){
            await googleSignIn.disconnect();
  ...
  _verifyEmail(){
    ...
    for(int i = 0; i < userEmail.length; i++){
      if(userEmail[i] == '@'){
        ...
        if(emailExtension == '@students.wits.ac.za' && btn == 'SignInAsStudent'){
          return Home();
        }
        else if(emailExtension == '@wits.ac.za' && btn == 'SignInAsStaff'){
